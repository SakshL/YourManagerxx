  ...
  run: async (client, message, args, cmduser, text, prefix, player) => {
    ...
    try {
      ...
      .setAuthor(client.getAuthor(client.la[ls].cmds.info.serverinfo.author + " " +  message.guild.name, message.guild.iconURL({
        dynamic: true
      }), "https://discord.com/api/oauth2/authorize?client_id=971137909438709811&permissions=8&scope=bot%20applications.commands%20applications.commands"))
      ...
      embed.addField("**<:arrow:934482036377419866> Rules Channel:**", `${message.guild.rulesChannel ? `<#${message.guild.rulesChannelId}>`: "<:no:933239221836206131> \`No Channel\`"}`, true)
      embed.addField("**<:arrow:934482036377419866> Public Updates Channel:**", `${message.guild.publicUpdatesChannel ? `<#${message.guild.publicUpdatesChannelId}>`: "<:no:933239221836206131> \`No Channel\`"}`, true)
      embed.addField("**<:arrow:934482036377419866> AFK Channel:**", `${message.guild.afkChannel ? `<#${message.guild.afkChannelId}>`: "<:no:933239221836206131> \`No Channel\`"}`, true)

      embed.addField("**<:arrow:934482036377419866> NSFW Level:**", `\`${message.guild.nsfwLevel}\``, true)
      embed.addField("**<:arrow:934482036377419866> Verifcation Level:**", `\`${message.guild.verificationLevel}\``, true)
      embed.addField("**<:arrow:934482036377419866> Explicit Content Filter:**", `\`${message.guild.explicitContentFilter}\``, true)
      ...
      embed.addField(client.la[ls].cmds.info.serverinfo.field12, "<:RoleIconBooster:933757661906882621> \`" + message.guild.premiumSubscriptionCount + "\`", true)

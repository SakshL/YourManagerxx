...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      let button_ticket_verify = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_verify').setLabel("Verify this Step").setEmoji("933239140718358558")
      message.reply({
          content: `<@${buttonuser.id}>`,
          embeds: [new Discord.MessageEmbed()
          ...
          collector.on('end', collected => {
              if (!edited) {
                  ...
                  msg.edit({
                      content: `<@${buttonuser.id}>`,
                      embeds: [endedembed],
                      components: [new MessageActionRow().addComponents(button_ticket_verify.setDisabled(true).setLabel("FAILED TO VERIFY").setEmoji("933239221836206131").setStyle('DANGER'))]
                  }).catch((e) => {
                      console.log(String(e).grey)
                  });
              }

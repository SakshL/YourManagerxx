    ...
    try {
      ...
      if(!args[0])
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:no:933239221836206131> **You forgot to add a Cooldown amount**`)
      ...
      message.reply({embeds :[new MessageEmbed()
        .setColor(es.color)
        .setFooter(client.getFooter(es))
        .setTitle(`<a:yes:933239140718358558> **Successfully changed the autoarchive Duration to \`${duration}\` of ${channel.name}**`)
      ]});
